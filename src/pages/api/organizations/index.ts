...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.course_offering?.length > 0) {
      const create_course_offering = body.course_offering;
      body.course_offering = {
        create: create_course_offering,
      };
    } else {
      delete body.course_offering;
    }
